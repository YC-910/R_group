...
# 1. Import Raw Data
df_raw <- read.csv("data/dairy_dataset.csv", stringsAsFactors = FALSE)
...
cat("CLEAN DATASET WITH FEATURE ENGINEERING")
str(df)
...
$$ quantity_sold_liters_kg = \beta_0 + \beta_1 quantity_liters_kg $$
$$ quantity_sold_liters_kg = \beta_0 + \beta_1 total_value + \beta_2 total_land_area_acres + \beta_3 number_of_cows + \beta_4 shelf_life_days $$
$$ quantity_sold_liters_kg = \beta_0 + \beta_1 quantity_in_stock_liters_kg + \beta_2 reorder_quantity_liters_kg + \beta_3 sales_channel + \beta_4 storage_condition $$
...
# --- Hypothesis 2 (MLR): Pre-Sale Valuation & Farm Assets ---
# Predictors: total_value, total_land_area_acres, number_of_cows, shelf_life_days
hyp2_mlr_valuation <- lm(quantity_sold_liters_kg ~ total_value + 
                            total_land_area_acres + 
                            number_of_cows + 
                            shelf_life_days, data = df)

# --- Hypothesis 3 (MLR): Inventory Balance & Distribution Channels ---
# Predictors: quantity_in_stock_liters_kg, reorder_quantity_liters_kg, sales_channel, storage_condition
hyp3_mlr_inventory <- lm(quantity_sold_liters_kg ~ quantity_in_stock_liters_kg + 
...
cat("\n--- HYPOTHESIS 2 SUMMARY: Pre-Sale Valuation & Farm Assets (MLR) ---\n")
print(summary(hyp2_mlr_valuation))

cat("\n--- HYPOTHESIS 3 SUMMARY: Inventory Balance & Distribution Channels (MLR) ---\n")
print(summary(hyp3_mlr_inventory))
...
hypothesis_comparison <- rbind(
  ...
  get_hypothesis_metrics(hyp2_mlr_valuation, "Hypothesis 2: Batch Valuation & Farm Assets (MLR)", df),
  get_hypothesis_metrics(hyp3_mlr_inventory, "Hypothesis 3: Inventory & Distribution Channels (MLR)", df)
)

# stargazer
stargazer(hyp1_slr_supply, hyp2_mlr_valuation, hyp3_mlr_inventory, type = "text", keep.stat = c("n", "adj.rsq"))
...
diagnostic_models <- list(
  "Hypothesis 1 (SLR)" = hyp1_slr_supply,
  "Hypothesis 2 (MLR)" = hyp2_mlr_valuation,
  "Hypothesis 3 (MLR)" = hyp3_mlr_inventory
)

# Run diagnostics for every model
for(model_name in names(diagnostic_models)){

  cat("\n=====================================================\n")
  cat("Diagnostic Results:", model_name, "\n")
  cat("=====================================================\n")

  diag_model <- diagnostic_models[[model_name]]

  # ----------------------------
  # VIF (only applicable for MLR)
  # ----------------------------
  if(length(attr(terms(diag_model), "term.labels")) > 1){
    ...
    cat("\n--- VIF ---\n")
    cat("Not applicable for Simple Linear Regression.\n")
  ...
  # Identify Top 5 Influential Observations for Plot Labeling
  threshold <- 4 / nrow(diag_df)
  top_cooks <- diag_df %>%
  ...
  print(top_cooks %>% select(obs_id, cooks_d, leverage, std_residuals))

  # Residuals vs Fitted
  print(
    ggplot(diag_df, aes(fitted, residuals)) +
      geom_point(alpha=.5, color="#1F78B4") +
  ...
  print(
    ggplot(diag_df, aes(sample=std_residuals)) +
      ...
      labs(title=paste(model_name, "- Normal Q-Q")) +
      theme_minimal()
  )

  # Scale-Location
  print(
    ggplot(diag_df, aes(fitted, sqrt_abs_std_res)) +
      geom_point(alpha=.5, color="#1F78B4") +
  ...
  print(
    ggplot(diag_df, aes(leverage, std_residuals)) +
      geom_point(alpha=.5, color="#1F78B4") +
      geom_hline(yintercept=0, linetype="dashed", color="red") +
  ...
  # Cook's Distance with Labeled Top Outliers
  print(
    ggplot(diag_df, aes(obs_id, cooks_d)) +
      geom_col(fill="#1F78B4") +
      geom_hline(yintercept=threshold, color="red", linetype="dashed") +
      geom_text(data=top_cooks, aes(label=paste("Row", obs_id)), 
                vjust=-0.5, color="darkred", fontface="bold", size=3.5) +
      labs(title=paste(model_name, "- Cook's Distance"),
           subtitle=paste("Red Line = 4/N Threshold (", round(threshold, 5), ")"),
           x="Observation (Row Number)",
           y="Cook's Distance") +
      theme_minimal()
  )
...
descriptive_summary_table <- data.frame(
  ...
  Key_Metric = c(
    ...
    paste("R² =", round(summary(hyp2_mlr_valuation)$r.squared, 4), "| RMSE =", round(rmse(df$quantity_sold_liters_kg, predict(hyp2_mlr_valuation, df)), 2)),
    paste("R² =", round(summary(hyp3_mlr_inventory)$r.squared, 4), "| RMSE =", round(rmse(df$quantity_sold_liters_kg, predict(hyp3_mlr_inventory, df)), 2)),
